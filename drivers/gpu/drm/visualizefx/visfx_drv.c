			       ...
			       struct drm_rect *rect)
{
	...
	iosys_map_set_vaddr_iomem(&dst, visfx->mmio + 0x01000000ULL);
...
static int visfx_fb_blit_fullscreen(struct drm_framebuffer *fb, const struct iosys_map *map)
{
...
static int visfx_mode_config_init(struct visfx_device *visfx)
{
	...
	dev->mode_config.preferred_depth = 8; /* actually: 32 */
...
static int visfx_conn_get_modes(struct drm_connector *conn)
{
	...
	drm_set_preferred_mode(conn, 1280, 1024);
	return count;
}
			   ...
			   const struct pci_device_id *ent)
{
	...
	visfx->mmio = devm_ioremap(&pdev->dev, pci_resource_start(pdev, 0),
